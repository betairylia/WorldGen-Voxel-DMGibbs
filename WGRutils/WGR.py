...
def GetBlock(x, y, z):
    r = requests.get("http://127.0.0.1:4444/?pos=%d,%d,%d" % (x, y, z))
    return int(r.text)

def SetBlock(x, y, z, block):
    r = requests.post("http://127.0.0.1:4444/?pos=%d,%d,%d&blk=%d" % (x, y, z, block))
...
def SetBlocks(x = None, y = None, z = None, block = None, bufferSize = 128):
    ...
    if (x is None) or (len(SetBlocks.buffer) >= bufferSize):
        ...
        # Send
        r = requests.post("http://127.0.0.1:4444/batched", data = reqText, headers={'Content-Type': 'raw'})
        SetBlocks.buffer = []


def GetBlockWithin(xMin, yMin, zMin, xMax, yMax, zMax):
    r = requests.get("http://127.0.0.1:4444/numpy?min=%d,%d,%d&max=%d,%d,%d" % (xMin, yMin, zMin, xMax+1, yMax+1, zMax+1))
    # print(r.text)
